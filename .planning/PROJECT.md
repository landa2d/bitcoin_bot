...
- ✓ **RESOLVED in Phase 17** (HUB-01): the hub block list renders ONCE as cards — the prose Tier-1/Tier-2 block-list is code-trimmed from the hub intro via `trimHubBody()`. `nascent` blocks render pill-only (no distinct visual treatment — Claude's-Discretion default, operator-approved; net-new visual capability is D-06 out-of-bounds).
