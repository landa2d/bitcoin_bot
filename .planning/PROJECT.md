...
- [ ] **REQ-GATE-DRAFT**: Synthesized bodies always land as `draft` versions; Telegram card surfaces flags; nothing goes live without `/map-approve` — *draft-landing shipped in Phase 7 (GATE-01); flag-surfacing shipped in Phase 8 (VLDT-05/06: sentinels annotate the draft's `validator_report` at insert and NEVER block creation; `/map-pending` renders `⚠ REQUIRES ATTENTION` + per-flag detail or `✓ clean`); `/map-approve` is Phase 9*
...
---
*Last updated: 2026-06-02 — Phase 8 complete (validation sentinels: `run_sentinels` computes four deterministic flags — tension-preserved, length-floor, maturity-jump, structure-intact — plus a `requires_attention` rollup, written ATOMICALLY into each draft's append-only `validator_report` at INSERT (D-02). Fail-loud-but-never-block (VLDT-05): a sentinel error logs `exc_info=True`, records `sentinel_errors`, forces attention — the draft always lands. The Phase 7 WR-02 missing-headings hard gate was converted to an annotating sentinel (D-01). `/map-pending` surfaces flags loudly (`⚠ REQUIRES ATTENTION` + serious-first per-flag detail, or `✓ clean`), read-only by construction. 5/5 must-haves verified, 29/29 synthesis tests. Code review: 0 Critical / 2 Warning / 3 Info; WR-01 (structure sentinel made heading-aware to close a silent-miss) + WR-02 + IN-03 fixed before verification; IN-01/IN-02 are documented v1 deferrals. Not yet deployed/running — awaits operator-approved scoped rebuild of processor + gato_brain. Next: Phase 9 — gated publishing + approval commands)*
