# Requirements Archive: v2.2 Landing Redesign + Signals Feed
...
- [x] **WIDTH-01**: On a wide viewport there is no large empty band on the left — content is centered via two coexisting max-widths: narrow prose (`--measure`, ~60–70 char lines) for edition body + intro copy, and a wider container (`--wide`) for the newsletter list, map grid, Signals, and card grids. _(Phase 20 P01: `.prose`/`.wide`/`--gutter` axes shipped, the 720px `.container` retired, nav widened to `--wide`; deployed live 2026-06-11; holistic re-verify folded into Phase 21. SATISFIED.)_
...
- [x] **RHYTHM-01**: No hardcoded colors — every surface themes from the existing CSS variable system (warm off-white + violet); section rhythm uses one full-strength rule between major sections and hairline (`0.5px`) rules within. _(Phase 20 P02: on-accent `#fff` literals aliased to `--on-accent`, D-05 section-rhythm hierarchy applied; deployed live 2026-06-11; net-new CSS across Phases 21–25 held token-only/hex-free. SATISFIED.)_
...
| SCROLL-01 | Phase 21 | Complete (single-scroll landing — 4 stacked sections, editions/blocks stay detail routes; deployed live 2026-06-11) |
| SCROLL-02 | Phase 21 | Complete (IntersectionObserver scroll-spy + detail→back scroll restore + reduced-motion gate; deployed live 2026-06-11) |
